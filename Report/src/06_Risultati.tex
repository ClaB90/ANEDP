...
I due esempi utilizzati come casi tests sono stati presi da \cite{MAIN}.
...
Il primo esempio considerato, in accordo con l'ipotesi i) introdotta nella sezione \ref{chap:Continuos}.
Preso il dominio spazio-temporale $\Omega \times I = (0,1)^2 \times (0,0.01)$ e d=1, si considera l'operatore di controllo lineare affine $\tilde{B}$ che può essere completamente caratterizato da:
...
\subsubsection{Set-up}
Il primo esempio considerato, in accordo con l'ipotesi i) introdotta in Chapter nella sezione \ref{chap:Continuos}.
Preso il dominio spazio-temporale $\Omega \times I = (0,1)^2 \times (0,0.5)$ e d=1, si considera l'operatore di controllo lineare affine $\tilde{B}$ che può essere completamente caratterizato da:
{\renewcommand\arraystretch{2}
\begin{equation}
\begin{array}{c}
g_1(t,x_1,x_2) = sin({\pi}x_1)sin({\pi}x_2)\\
...
La perdita riscontrata nell'ordine di convergenza per l'ultima mesh temporale è invece da attribuire all'avvicinamento con la griglia spaziale. (EDO)
\par
...
\label{secondanorma}
\end{equation}
Col criterio \ref{primanorma} l'algoritmo converge in tre passi per ogniuna delle mesh considerate. Mentre utilizzano \ref{secondanorma} si ha la convergenza dopo nove passi per la prima griglia utilizzata e mentre quattro dalla successiva in avanti.
